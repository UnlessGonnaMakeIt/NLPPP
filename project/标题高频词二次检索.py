# -*- coding: utf-8 -*-
"""
Created on Sat Jan  4 11:49:28 2025
...
# 结果保存路径
result_folder_path = 'D:/A NLP/storage/dh/research results/title'
...
def find_word_occurrences(word, file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        title = file.readline().strip()  # 只读取文件的第一行
    positions = [m.start() for m in re.finditer(r'\b' + re.escape(word.lower()) + r'\b', title.lower())]
    return positions
...
def search_word():
    ...
    # 输出为 HTML 格式
    output_html_path = os.path.join(result_folder_path, f"{search_word}_search_results_title.html")
      ...
      .file-content {
        ...
        for _, row in matching_rows.iterrows():
            ...
            if file_occurrences:
                # 提取标题内容并排序
                titles_list = []
                for file, _ in file_occurrences:
                    file_path = os.path.join(folder_path, file)
                    with open(file_path, 'r', encoding='utf-8') as file:
                        title = file.readline().strip()
                        titles_list.append(title)
                sorted_titles = sorted(titles_list, key=lambda x: x.lower().count(search_word.lower()))

                # 生成词云
                text_for_wordcloud = " ".join(sorted_titles)
                wordcloud = WordCloud(width=800, height=400, background_color='white').generate(text_for_wordcloud)
                # 保存词云图片到指定路径（这里使用之前定义的词云图片文件夹路径）
                wordcloud_image_path = os.path.join(wordcloud_folder_path, f"{search_word}_wordcloud.png")
                wordcloud.to_file(wordcloud_image_path)

                # 生成词频表格
                word_frequency_table_df = generate_word_frequency_table(text_for_wordcloud)
                # 保存词频表格为 Excel 文件
                table_save_path = os.path.join(excel_folder_path, "search_result", f"{search_word}_word_frequency_table.xlsx")
                word_frequency_table_df.to_excel(table_save_path, index=False)

                # 生成词频表格的 HTML 内容
                word_frequency_table_html = generate_word_frequency_table_html(word_frequency_table_df)
        ...
        # 显示总出现次数
        html_file.write(f"</ul><p><b>词汇 '{search_word}' 出现在标题中的总次数: {total_occurrences}</b></p>")

        # 在HTML中添加展示词云图片和词频表格的部分
        html_file.write(f"""
        <div style="display: flex;">
            <img src='{wordcloud_image_path}' alt='词云图' style='width: 500px; height: 300px; margin-top: 20px; margin-right: 20px;'>
            <div style="width: 500px; height: 300px; overflow: auto;">
                {word_frequency_table_html}
            </div>
        </div>
        """)
...
def generate_word_frequency_table(text):
    ...
    df = pd.DataFrame({'Word': word_list, 'Frequency': count_list})
    return df


def generate_word_frequency_table_html(df):
    """
    将 DataFrame 转换为 HTML 表格格式的字符串
    """
    table_html = "<table border='1'><tr><th>单词</th><th>词频</th></tr>"
    for index, row in df.iterrows():
        table_html += f"<tr><td>{row['Word']}</td><td>{row['Frequency']}</td></tr>"
    table_html += "</table>"
    return table_html
